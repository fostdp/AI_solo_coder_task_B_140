...
public class VirtualLoadingService {
    ...
    public VirtualLoadingResultDTO createSession(VirtualLoadingCreateRequest request) {
        log.info("[VirtualLoading] 创建新会话 - 船舶: {}, 用户: {}, 名称: {}",
        ...
        StabilityResultDTO stabilityResult = calculateVirtualStability(ship, session.getLoadingConfig());
        session.setCurrentGm(stabilityResult.getGmValue());
        session.setStabilityStatus(stabilityResult.getStabilityStatus());

        session = sessionRepository.save(session);
        log.info("[VirtualLoading] 会话创建成功 - ID: {}", session.getId());
    ...
    public VirtualLoadingResultDTO executeAction(VirtualLoadingActionRequest request) {
        log.info("[VirtualLoading] 执行操作 - 会话: {}, 操作: {}", request.getSessionId(), request.getAction());
        ...
        StabilityResultDTO stabilityResult = calculateVirtualStability(ship, loadingConfig);
    ...
    private String handleLoad(VirtualLoadingSession session, Map<String, Map<String, BigDecimal>> loadingConfig,
        ...
        String validationError = validateLoading(holdId, cargoTypeId, currentWeight, weightChange);
    ...
    private String handleUnload(VirtualLoadingSession session, Map<String, Map<String, BigDecimal>> loadingConfig,
        ...
        if (holdConfig == null || !holdConfig.containsKey(cargoTypeId.toString())) {
            return "该货舱中没有指定类型的货物";
        }

        BigDecimal currentWeight = holdConfig.get(cargoTypeId.toString());
        if (currentWeight.compareTo(unloadWeight) < 0) {
            return String.format("卸载量(%.2f吨)超过当前装载量(%.2f吨)", unloadWeight, currentWeight);
        }

        BigDecimal newWeight = currentWeight.subtract(unloadWeight);
    ...
    private String handleResetHold(VirtualLoadingSession session, Map<String, Map<String, BigDecimal>> loadingConfig,
        ...
        if (request.getHoldId() == null) {
            throw new IllegalArgumentException("重置货舱需要指定holdId");
        }
    ...
    private void validateActionParameters(VirtualLoadingActionRequest request) {
        if (request.getHoldId() == null) {
            throw new IllegalArgumentException("货舱ID不能为空");
        }
        if (request.getCargoTypeId() == null) {
            throw new IllegalArgumentException("货物类型ID不能为空");
        }
        if (request.getWeightChange() == null) {
            throw new IllegalArgumentException("重量变化不能为空");
        }
    }
    ...
    public String validateLoading(UUID holdId, UUID cargoTypeId, BigDecimal currentWeight, BigDecimal addWeight) {
        ...
        BigDecimal newWeight = currentWeight.add(addWeight);
        if (newWeight.compareTo(hold.getMaxWeight()) > 0) {
            return String.format("装载后重量(%.2f吨)超过货舱最大承重(%.2f吨)",
                    newWeight, hold.getMaxWeight());
        }

        BigDecimal newVolume = newWeight.divide(cargoType.getDensity(), 2, RoundingMode.HALF_UP);
        if (newVolume.compareTo(hold.getCapacityCubic()) > 0) {
            return String.format("装载后容积(%.2fm³)超过货舱容积(%.2fm³)",
                    newVolume, hold.getCapacityCubic());
        }
    ...
    private StabilityResultDTO calculateVirtualStability(Ship ship, Map<String, Map<String, BigDecimal>> loadingConfig) {
        ...
        log.debug("[VirtualLoading] 虚拟稳性计算完成 - 耗时: {}ms, GM: {}m", duration, gmTransverse);
    ...
    public VirtualLoadingResultDTO getSession(UUID sessionId) {
        log.info("[VirtualLoading] 获取会话详情 - ID: {}", sessionId);
    ...
    public List<VirtualLoadingResultDTO> getActiveSessions(UUID shipId) {
        log.info("[VirtualLoading] 获取船舶活跃会话 - 船舶: {}", shipId);
    ...
    public Page<VirtualLoadingResultDTO> getPublicSessions(int page, int size) {
        log.info("[VirtualLoading] 获取公开会话 - 页码: {}, 大小: {}", page, size);
    ...
    public List<VirtualLoadingResultDTO> getUserSessions(String userId) {
        log.info("[VirtualLoading] 获取用户会话 - 用户: {}", userId);
        List<VirtualLoadingSession> sessions = sessionRepository
    ...
    public void closeSession(UUID sessionId) {
        log.info("[VirtualLoading] 关闭会话 - ID: {}", sessionId);
        VirtualLoadingSession session = sessionRepository.findById(sessionId)
                .orElseThrow(() -> new IllegalArgumentException("会话不存在: " + sessionId));
    ...
    public VirtualLoadingResultDTO cloneSession(UUID sessionId, String newName) {
        log.info("[VirtualLoading] 克隆会话 - 源ID: {}, 新名称: {}", sessionId, newName);
        ...
        StabilityResultDTO stabilityResult = calculateVirtualStability(ship, cloned.getLoadingConfig());
