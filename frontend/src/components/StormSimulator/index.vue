<template>
  <div class="storm-simulator" :class="{ 'danger-flash': showDangerFlash }">
...
const emit = defineEmits(['simulate', 'batch-simulate'])
...
const setSimulationResult = (result, shipId = null, batch = []) => {
  if (batch.length > 0) {
    batchResults.value = batch
    simulationResult.value = batch[0] || null
  } else if (result) {
    simulationResult.value = result
    batchResults.value = shipId ? [{ ...result, shipId }] : []
  }
  nextTick(() => {
    initAllCharts()
  })
...
defineExpose({ setSimulationResult })
...
const startSimulation = async () => {
  ...
  try {
    ...
    if (selectedShips.value.length === 1) {
      emit('simulate', selectedShips.value[0], params)
    } else {
      const request = {
        shipIds: selectedShips.value,
        ...params
      }
      emit('batch-simulate', request)
    }
