    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="header-desc">
          多船型稳性对比，支持古代沙船/福船/广船与现代散货船跨代对比
        </div>
      </div>
          ...
          <div class="card-body">
            <el-form :model="comparisonConfig" label-width="110px">
              ...
              <el-form-item label="对比指标">
                <el-checkbox-group v-model="comparisonConfig.comparisonCriteria">
                  <el-row :gutter="8">
                    <el-col :span="12">
              ...
              <el-form-item label="装载条件">
                <el-radio-group v-model="comparisonConfig.loadingCondition" size="default">
    ...
    >
      <div class="history-list">
        ...
        <div
          ...
          :key="item.id"
          class="history-item"
          @click="loadComparison(item.id)"
        >
          <div class="history-header">
            <span class="history-name">{{ item.comparisonName }}</span>
            <el-button
              type="danger"
              size="small"
              text
          ...
          <div class="history-condition">
            装载条件: {{ getLoadingConditionText(item.loadingCondition) }} |
            波高: {{ item.referenceWaveHeight }}m
          </div>
        </div>
      </div>
    </el-drawer>
...
import {
  Scale, History, Setting, Operation, Refresh, Collection,
  DataLine, Trophy, Histogram, PieChart, BarChart,
  ChatDotRound, Ship, Box, Timer, Loading, Delete,
  Compass, Crop, Van, Flag
} from '@element-plus/icons-vue'
import { Chart, registerables } from 'chart.js'
import {
  getAvailableShips, compareShips, getComparisonHistory,
  getComparisonById, deleteComparison
} from '@/api/shipComparison'
...
const runComparison = async () => {
  ...
  try {
    ...
    const res = await compareShips(request)
    comparisonResult.value = res.data
    ElMessage.success('对比计算完成')
...
const loadAvailableShips = async () => {
  try {
    const res = await getAvailableShips()
    availableShips.value = res.data || {}
  } catch (e) {
...
const openHistoryDrawer = async () => {
  ...
  try {
    const res = await getComparisonHistory(20)
...
const loadComparison = async (id) => {
  try {
    ...
    comparisonConfig.comparisonName = res.data.comparisonName || ''
    comparisonConfig.loadingCondition = res.data.loadingCondition || 'FULL_LOAD'
    comparisonConfig.referenceWaveHeight = res.data.referenceWaveHeight || 3
    comparisonConfig.comparisonCriteria = res.data.comparisonCriteria || ['GM', 'GZ_MAX', 'RANGE', 'DEADWEIGHT', 'ROLL_PERIOD']
    comparisonConfig.shipIds = res.data.comparisonItems?.map(i => i.shipId) || []
    ...
    ElMessage.error('加载对比记录失败: ' + (e.message || '未知错误'))
  }
}
